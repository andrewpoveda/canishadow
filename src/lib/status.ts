// Ported verbatim from reference-base44/src/lib/status.js — the design "brain."
// Values (COLORS, STATUS_META, PIN_ORDER) and the effectiveStatus() rule are unchanged;
// only TypeScript signatures were added so it compiles clean under strict mode.
import type { ClinicStatus, Provider } from "@/types/clinic";

export type EffectiveStatus =
  | "unknown"
  | "call_back"
  | "verified_no"
  | "verified_yes";

export interface StatusMeta {
  fill: string;
  strokeWeight: number;
  radius: number;
  halo: string | null;
  badgeLabel: string;
  badgeClass: string;
  railClass: string;
}

export const COLORS = {
  paper: "#FAF9F6",
  paper2: "#F1EFEA",
  ink: "#0F1B2D",
  ink3: "#8A93A6",
  verified: "#0F5144",
  verifiedTint: "#E4EFEA",
  declined: "#8E3B3B",
  callback: "#9A7B2D",
  callbackTint: "#F4EDDB",
  line: "#E3E0D8",
};

export const STATUS_META: Record<EffectiveStatus, StatusMeta> = {
  unknown: {
    fill: COLORS.ink3,
    strokeWeight: 1.5,
    radius: 4,
    halo: null,
    badgeLabel: "Unverified",
    badgeClass: "bg-paper-2 text-ink-2",
    railClass: "bg-line",
  },
  call_back: {
    fill: COLORS.callback,
    strokeWeight: 2,
    radius: 5,
    halo: null,
    badgeLabel: "Call back in a few months",
    badgeClass: "bg-callback-tint text-callback",
    railClass: "bg-callback",
  },
  verified_no: {
    fill: COLORS.declined,
    strokeWeight: 2,
    radius: 5,
    halo: null,
    badgeLabel: "Not taking students",
    badgeClass: "bg-declined-tint text-declined",
    railClass: "bg-declined",
  },
  verified_yes: {
    fill: COLORS.verified,
    strokeWeight: 2,
    radius: 6,
    halo: COLORS.verifiedTint,
    badgeLabel: "Takes shadowing students",
    badgeClass: "bg-verified-tint text-verified",
    railClass: "bg-verified",
  },
};

export const PIN_ORDER: EffectiveStatus[] = [
  "unknown",
  "call_back",
  "verified_no",
  "verified_yes",
];

/** Pin/badge status — green if at least one provider said yes, regardless of clinic-level status. */
export function effectiveStatus(clinic: {
  providers?: Provider[] | null;
  status?: ClinicStatus | null;
}): EffectiveStatus {
  if ((clinic.providers || []).some((p) => p.response === "yes"))
    return "verified_yes";
  return (clinic.status as EffectiveStatus) || "unknown";
}
